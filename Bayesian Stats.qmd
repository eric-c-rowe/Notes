...
Example involves identifying proportions of folks selected for a program. The selection probability depends on their education and pay plan, and the highly educated people are more likely to be in the pay plan with the high selection probability. The goal is to correctly identify the different probabilities. 

```{r}
df <- tibble(
  x = 1:1e4,
  edu = rbinom(1e4, 1, .3), # 1 = 'highly educated', 0 = 'normal',
  edu_grp = ifelse(edu == 1, 'high_edu', 'low_edu'),
  pp = rbinom(1e4, 1, ifelse(edu == 1, .2, .05)), # chance of being CC dep on edu
  pp_grp = ifelse(pp == 1, 'CC', 'GS'),
  base_prob = .15,
  edu_prob = ifelse(edu == 1, .2, 0),
  pp_add = ifelse(pp == 1, .35, 0),
  prob = edu_prob + pp_add + base_prob,
  y = rbinom(1e4, 1, prob)
)
```
